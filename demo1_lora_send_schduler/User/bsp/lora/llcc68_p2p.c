 ...
 *   工作流程：
 *   1. llcc68_init() 初始化模块
 *   2. 初始化各传感器（DHT22、CO2、光敏、土壤湿度）
 *   3. 每3秒读取传感器数据并通过LoRa发送（4个int16_t，共8字节）
 *   4. 发送完成时DIO1上升沿 → EXTI中断 → DIO1_EXTI_Callback()
 *
 *   依赖文件：
...
void DIO1_EXTI_Callback(void)
{
	...
	{
		tx_done = true;  /* 设置发送完成标志（bool类型，使用true与定义一致） */
		printf("IRQ Tx done\r\n");
	}
                                         ...
                                         llcc68_pkt_status_lora_t *pkt_status, uint32_t timeout_in_ms)
{
    ...
    while (!rx_done && !rx_timeout)
    {
        ...
        {
            rx_timeout = false;  /* 清除超时标志（原笔误写成tx_done，与接收逻辑无关） */

 ...
 *          3. 常用接口：
 *             - photores_get_voltage()：返回传感器电压（mV）
...
uint16_t photores_get_lux(void)
{
	...
	uint32_t resistance = (uint32_t)(10.0f * 1000 * adc_voltage) / (3300 - adc_voltage); /* 由 10kΩ 分压电阻反算光敏电阻阻值(Ω) */

	/* 遍历结构体数组，提取光强：
	 * 从"高电阻=最暗"开始比较，实际电阻>=当前表项则命中该等级
	 * 若循环完所有表项（电阻极低=极强光），lux 保持为最后一项 350 */

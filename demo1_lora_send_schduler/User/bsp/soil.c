 ...
 * @param  无
 * @return 湿度百分比（0=干燥，100=水分饱和）
 * @note   传感器特性：土壤越湿 → 阻值越小 → 分压输出越低。
 *         故 moisture% = (3300 - 电压mV) / 3300 * 100。
 *         直接读取 DMA 连续采样表 g_adc_value_tbl[1]（PA1/ADC1_IN1），
 *         不打印，避免污染 LoRa 发送日志。
 */
uint16_t soil_get_humi_percent(void)
{
	uint16_t adc_voltage; /* 保存电压值（单位 mV） */
	uint16_t percent;	  /* 保存湿度百分比 */

	/* 由 DMA 连续采样的 ADC 原始值换算电压（mV），土壤接 PA1=通道1 */
	adc_voltage = (uint16_t)((uint32_t)g_adc_value_tbl[1] * 3300UL / 4095);

	/* 电压越高越干燥，电压越低越湿润（线性映射为 0~100%） */
	if (adc_voltage >= 3300)
		percent = 0;
	else
		percent = (uint16_t)((uint32_t)(3300 - adc_voltage) * 100UL / 3300);

	return percent; /* 返回湿度百分比（0~100） */
}
